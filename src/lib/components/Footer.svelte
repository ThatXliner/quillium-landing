...
<footer class="py-12 px-8 text-center border-t border-black/4">
	...
	<div class="mx-auto max-w-[20rem] mb-8">
		{#if !submitted}
			<p class="mb-3 text-[0.75rem] text-black/40">Get release notes &amp; updates</p>
			<form onsubmit={handleSubmit} class="flex gap-2 max-[400px]:flex-col">
				<input
					...
					bind:value={email}
					required
					class="min-w-0 flex-1 rounded-lg border border-black/8 bg-white/60 px-3 py-2 font-[Inter,sans-serif] text-[0.8rem] text-black/88 transition-[border-color] duration-200 placeholder:text-black/25 focus:border-[#3b82f6] focus:ring-[3px] focus:ring-[rgba(59,130,246,0.1)] focus:outline-none"
					aria-label="Email address"
				/>
				<button
					type="submit"
					class="shrink-0 rounded-lg border border-black/10 bg-white/70 px-4 py-2 font-[Inter,sans-serif] text-[0.75rem] font-medium text-black/60 transition-colors duration-200 hover:bg-white hover:text-black/88 disabled:opacity-50 disabled:cursor-not-allowed"
	...
	<div class="flex flex-wrap justify-center gap-x-8 gap-y-2 mb-6">
		...
		<a href="/terms" class="text-[0.75rem] text-black/50 no-underline transition-colors duration-300 hover:text-black/88">Terms</a>
		...
		<a href="/rss.xml" class="text-[0.75rem] text-black/50 no-underline transition-colors duration-300 hover:text-black/88">RSS</a>
	</div>
