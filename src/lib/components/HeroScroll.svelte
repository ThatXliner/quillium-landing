<script lang="ts">
	...
	onMount(() => {
		...
		tl.to(
			shotWrap,
			{ x: 0, y: 0, scale: 1, xPercent: -50, yPercent: -50, duration: 0.14, ease: 'power2.inOut' },
			0.06
		);
			...
			<div class="text-col" id="text-col">
				...
				<h1 class="headline">
					{displayText}{#if showPros}<span class="italic">Pros</span
						>{/if}{#if showPeriod}.{/if}<span class="cursor" class:hidden={!showCursor}>|</span>
				</h1>
				<p class="intro-desc">
				...
				<div class="copy-strip">
					...
					<div class="copy-block" id="copy-branches">
						<p class="copy-subtitle">
							Fork any sentence. Keep every version. Navigate your creative decisions freely and try
							what might work.
						</p>
					</div>
					...
					<div class="copy-block" id="copy-safety">
						<p class="copy-subtitle">
							Your work is saved locally — durable, reliable, instant. Even if your computer crashes
...
<style>
	...
	.shot {
		...
		border-radius: 12px;
		box-shadow:
			0 16px 64px rgba(44, 42, 39, 0.1),
			0 4px 16px rgba(44, 42, 39, 0.05);
	}
